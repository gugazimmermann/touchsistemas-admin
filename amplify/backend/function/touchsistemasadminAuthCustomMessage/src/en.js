async function signUpMessage(event, generateEmailBody) {
	event.response = {
      ...
      <br />
      <p>Enter your code in the field provided or <a href="${process.env.ADMIN_URL}confirm-registration?email=${event.request.userAttributes.email}&code=${event.request.codeParameter}">click here</a>.</p>
      `),
	};
	return event;
}
...
async function forgotPassword(event, generateEmailBody) {
	event.response = {
      ...
      <br />
      <p>Enter your code in the field provided or <a href="${process.env.ADMIN_URL}reset-password?email=${event.request.userAttributes.email}&code=${event.request.codeParameter}">click here</a>.</p>
      `),
	};
	return event;
}
