async function signUpMessage(event, generateEmailBody) {
	event.response = {
      ...
      <br />
      <p>Introduzca su código en el campo proporcionado o <a href="${process.env.ADMIN_URL}confirmar-registro?email=${event.request.userAttributes.email}&code=${event.request.codeParameter}">clic aquí</a>.</p>
      `),
	};
	return event;
}
...
async function forgotPassword(event, generateEmailBody) {
	event.response = {
      ...
      <br />
      <p>Introduzca su código en el campo proporcionado o <a href="${process.env.ADMIN_URL}redefinir-contrasena?email=${event.request.userAttributes.email}&code=${event.request.codeParameter}">clic aquí</a>.</p>
      `),
	};
	return event;
}
