...
export default function SubscriptionCard({ event }) {
	...
	return (
		<Link to={`${ROUTES[state.lang].SUBSCRIPTIONS}/${event.id}`} key={event.id} className="flex flex-col justify-between shadow-md rounded-lg">
			<div className="w-full bg-gray-200 rounded-t-lg overflow-hidden">
				{event.image ? (
					<img src={event.image} alt={event.name} className="w-full h-full object-center object-cover group-hover:opacity-75" />
				) : (
					<img
