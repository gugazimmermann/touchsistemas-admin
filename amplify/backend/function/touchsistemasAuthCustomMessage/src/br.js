async function signUpMessage(event, generateEmailBody) {
	event.response = {
      ...
      <br />
      <p>Digite seu código no campo informado ou <a href="${process.env.ADMIN_URL}confirmar-cadastro?lang=${event.request.userAttributes.locale.toLocaleLowerCase()}&email=${event.request.userAttributes.email}&code=${event.request.codeParameter}">clique aqui</a>.</p>
      `),
	};
	return event;
}
...
async function forgotPassword(event, generateEmailBody) {
	event.response = {
      ...
      <br />
      <p>Digite seu código no campo informado ou clique aqui: <a href="${process.env.ADMIN_URL}redefinir-senha?lang=${event.request.userAttributes.locale.toLocaleLowerCase()}&email=${event.request.userAttributes.email}&code=${event.request.codeParameter}">clique aqui</a>.</p>
      `),
	};
	return event;
}
